$(function () {
    ...
    var res = pgurl.split(mainUrl);
  
    if (res.length != 0){
	    var $activeMenu = $('.sidebar-menu').find("a[href$='"+res[1]+"']");
	    var $menu = $activeMenu;
	    
	    while ($menu.parent().is('li') || $menu.parent().is('ul')) {
	    	if ($menu.parent().is('li')) $menu.parent().addClass("active");
	    	if ($menu.parent().is('ul')) $menu.parent().addClass("in");
	    	$menu = $menu.parent();
		}
    }
